...
def index_to_position(index, strides):
    ...
    #    posicion = posicion + index[i] * strides[i]
...
def count(position, shape, out_index):
    ...
    for strid in strides:
        act_indice = nuev_pos // strid
        out_index[i] = act_indice
        nuev_pos = nuev_pos-strid*act_indice
        i += 1
...
def shape_broadcast(shape1, shape2):
    ...
    maxlen = max(len_sh1, len_sh2)

    rev_union = [0] * maxlen

    for i in range(maxlen):
        if i >= len_sh1:
            rev_union[i] = rev_shape2[i]

        elif i >= len_sh2:
            rev_union[i] = rev_shape1[i]
        else:
            rev_union[i] = max(rev_shape1[i], rev_shape2[i])
            if rev_shape1[i] != rev_shape2[i] and rev_shape1[i] != 1 and rev_shape2[i] != 1:
                raise IndexingError("Failed broadcast")

    return tuple(reversed(rev_union))
...
class TensorData:
    ...
    def permute(self, *order):
        ...
        newStride = tuple([self._strides[i] for i in order])
